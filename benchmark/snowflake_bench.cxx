...
/// stays stable
options hot_options(algo which) {
    options o;
    o.algo = which;
...
void BM_next_id_single_thread(benchmark::State& state, algo which) {
...
BENCHMARK_CAPTURE(BM_next_id_single_thread, drift, algo::drift);
BENCHMARK_CAPTURE(BM_next_id_single_thread, original, algo::original);

void BM_next_id_contended(benchmark::State& state, algo which) {
    // one process-wide generator per algo; threads contend on the same mutex
    static basic_generator<system_clock_source> drift_gen(hot_options(algo::drift));
    static basic_generator<system_clock_source> original_gen(hot_options(algo::original));
    auto& gen = which == algo::drift ? drift_gen : original_gen;
    for ([[maybe_unused]]
         const auto _ : state) {
        benchmark::DoNotOptimize(gen.next_id());
    }
...
BENCHMARK_CAPTURE(BM_next_id_contended, drift, algo::drift)
    ->UseRealTime()
    ->Threads(2)
    ->Threads(4)
    ->Threads(8);
BENCHMARK_CAPTURE(BM_next_id_contended, original, algo::original)
    ->UseRealTime()
    ->Threads(2)
    ->Threads(4)
    ->Threads(8);
...
void BM_validate_options(benchmark::State& state) {
    const options opts = hot_options(algo::drift);
    for ([[maybe_unused]]
         const auto _ : state) {
...
void BM_decode(benchmark::State& state) {
    basic_generator<system_clock_source> gen(hot_options(algo::drift));
